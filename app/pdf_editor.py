...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        merge_button_output_path.clicked.connect(self.browse_merge_output_path)
    ...
    def re_render(self):
        ...
        for i in range(len(self.elements_list)):
            self.main_form.addRow(self.elements_list[i]["label"],
                                              self.elements_list[i]["element"])
    ...
    def perform_button_action(self, button_id):
        ...
        if action == "up":
            ...
            if line_edit_content:
                file_path, _ = QFileDialog.getOpenFileName(self, f"Select PDF {row_id}", line_edit_content,
                                                           "PDF (*.pdf)")
            else:
                user_dir = os.path.join(os.path.expanduser("~"), "Documents")
                file_path, _ = QFileDialog.getOpenFileName(self, f"Select PDF {row_id}", user_dir,
                                                           "PDF (*.pdf)")
            if file_path:
                line_edit.setText(file_path)

    def browse_merge_output_path(self):
        line_edit = self.findChild(QLineEdit, "merge_lineEdit_output")
        line_edit_content = line_edit.text()
        if line_edit_content:
